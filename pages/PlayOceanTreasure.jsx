...
import OceanTreasureSlot from '@/components/slots/OceanTreasureSlot';
import WalletDisplay from '@/components/casino/WalletDisplay';

export default function PlayOceanTreasure() {
  ...
  const handleBet = async (betAmount, winAmount, gameSlug) => {
    ...
    if (betAmount > 0) {
      ...
      await updateWalletMutation.mutateAsync({ id: wallet.id, data: { [balanceKey]: newBalance, total_lost: (wallet.total_lost || 0) + betAmount } });
    }
    if (winAmount > 0) {
      ...
      createTransactionMutation.mutate({ user_email: user.email, type: 'win', amount: winAmount, game_slug: gameSlug, description: `Won ${winAmount} tokens on ${gameSlug}`, balance_after: newBalance });
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3">
            <OceanTreasureSlot wallet={wallet} onBet={handleBet} currency={currency} />
          </div>
