...
import StellarRushGame from '@/components/games/StellarRushGame';
...
export default function PlayStellarRush() {
  ...
  const updateWalletMutation = useMutation({
    ...
    onError: (err, variables, context) => {
      if (context?.previousWallets) queryClient.setQueryData(['wallet', user?.email], context.previousWallets);
    },
  ...
  const handleBet = async (betAmount, winAmount, gameSlug) => {
    ...
    if (winAmount > 0) {
      ...
      createTransactionMutation.mutate({
        ...
        description: `Won ${winAmount.toFixed(2)} ${currency} on Stellar Rush`, balance_after: newBalance,
      });
    ...
    <div className="min-h-screen bg-[#0A0612]" style={{ zoom: '0.85' }}>
      <div className="sticky top-0 z-50 bg-[#0A0612]/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <h1 className="text-2xl font-bold text-white">🧑‍🚀 Stellar Rush</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3">
            <StellarRushGame wallet={wallet} onBet={handleBet} currency={currency} />
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="bg-[#1c1c2e] rounded-2xl overflow-hidden">
          ...
          >
            <span className="text-lg font-bold">Stellar Rush</span>
            ...
            <div className="px-6 pb-6 space-y-5 border-t border-white/10 pt-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['# Space', '# Imperial Originals', '# Crash Games'].map(tag => (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className="text-gray-400 text-sm">Difficulties</p><p className="text-pink-400 font-bold text-lg">4 Levels</p></div>
                <div><p className="text-gray-400 text-sm">Max Win</p><p className="text-pink-400 font-bold text-lg">Unlimited</p></div>
              </div>
              <p className="text-gray-400 text-sm leading-relaxed">
                Stellar Rush is an exciting, space-themed crash-style game. Guide your astronaut through the galaxy, navigating past planets to hit increasing multipliers. Choose your difficulty — from Easy to Daredevil — and decide when to Cash Out or risk it all by continuing to the next planet. The higher the difficulty, the bigger the rewards!
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { d: 'Easy', chance: '80%', color: 'text-green-400' },
                  { d: 'Medium', chance: '65%', color: 'text-yellow-400' },
                  { d: 'Hard', chance: '48%', color: 'text-orange-400' },
                  { d: 'Daredevil', chance: '28%', color: 'text-red-400' },
                ].map(({ d, chance, color }) => (
                  <div key={d} className="bg-white/5 rounded-lg p-3 text-center">
                    <p className={`font-bold ${color}`}>{d}</p>
                    <p className="text-gray-400 text-xs mt-1">{chance} per planet</p>
                  </div>
                ))}
              </div>
            </div>
